...
impl<'a> ProofNode<'a> {
    ...
    #[inline(always)]
    fn new(seq: Sequent<'a>, parent_idx: usize) -> ProofNode<'a> {
        ProofNode { seq, tactic: OnceCell::new(), proved_children_cnt: 0, parent_idx: Some(parent_idx) }
    }
}
...
fn forest_impl(seq: Sequent, names: &Names, buf: &mut Vec<u8>) -> io::Result<()> {
    ...
    'main: loop {
        ...
        match (fml, side) {
            ...
            (Not(p), _) => {
                ...
                seq.push(p);
                let seq = ProofNode::new(seq, nodes.len() - 1);
                if is_trivial {
            ...
            (To(p, q), Left) => {
                ...
                seq2.push(p);
                let parent_idx = nodes.len() - 1;
                let seq1 = ProofNode::new(seq1, parent_idx);
                let seq2 = ProofNode::new(seq2, parent_idx);
                if is_trivial_q {
                    // if trivial, set the Axiom tactic
