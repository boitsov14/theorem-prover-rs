    ...
    fn apply(self, mut sequent: Sequent) -> Vec<(Sequent, ProofState)> {
        ...
        match self {
            ...
            RImplies => {
                ...
                use ProofState::*;
                match (sequent.insert_to_ant(*p), sequent.insert_to_suc(*q)) {
                    (Provable, _) | (_, Provable) => {
                        vec![(sequent, Provable)]
                    }
                    _ => {
                        vec![(sequent, InProgress)]
                    }
                }
            }
            LAnd => {
            ...
            ROr => {
                let Or(l) = sequent.suc.or_set.pop().unwrap();
                let mut state = ProofState::InProgress;
                for p in l {
                    if let ProofState::Provable = sequent.insert_to_ant(p) {
                        state = ProofState::Provable;
            ...
            LImplies => {
                let Implies(p, q) = sequent.ant.implies_set.pop().unwrap();
